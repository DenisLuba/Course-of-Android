...
import com.example.p011_handler.databinding.ActivityHandlerBinding
        ...
        Thread {
            when (it.id) {
                ...
                R.id.enableDisableButton -> {
                    val message: Message = handler.obtainMessage(MSG_TOGGLE_BUTTON)
                ...
                R.id.randomColorTokenDelayedButton -> {
                    val message: Message = handler.obtainMessage(MSG_NEXT_RANDOM_COLOR)
                    message.obj = token
                    handler.sendMessageDelayed(message, DELAY)
                }
                R.id.showToastButton -> {
                    val message: Message = handler.obtainMessage(MSG_SHOW_TOAST)
                    message.obj = token
                    handler.sendMessageDelayed(message, DELAY)
                }

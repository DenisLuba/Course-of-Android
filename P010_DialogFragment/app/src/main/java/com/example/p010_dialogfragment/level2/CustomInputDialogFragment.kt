...
import com.example.p010_dialogfragment.databinding.PartVolumeInputBinding
...
class CustomInputDialogFragment : DialogFragment() {
    ...
    companion object {
        ...
        fun showDialog(manager: FragmentManager, volume: Int, requestKey: String) {
        ...
        fun setupListener(manager: FragmentManager, lifecycleOwner: LifecycleOwner, requestKey: String, listener: CustomInputDialogListener) {
            manager.setFragmentResultListener(requestKey, lifecycleOwner, FragmentResultListener { requestKey, result ->
                listener.invoke(requestKey, result.getInt(KEY_VOLUME_RESPONSE))
            })
        }
    }
}